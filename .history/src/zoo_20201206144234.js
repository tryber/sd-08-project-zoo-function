...
function schedule(dayName) {
  ...
  if (typeof dayName !== "string") {
    daysAsked = Object.getOwnPropertyNames(calendar);
  } else {
    daysAsked.pusch(dayName);
  }
  console.table(daysAsked);
  console.log(calendar.Friday.open)
  result = daysAsked.forEach((day) => {
  for (const [key, value] of Object.entries(calendar)) {
    if (day.open === 0 && key === day) {
      console.log(` ${key}: 'CLOSED'`)
      return ` ${key}: 'CLOSED'`;
    }else if(key === day) {
      console.log(` ${key}: Open from ${value.open}am util ${value.close} `)
      return ` ${key}: Open from ${value.open}am util ${value.close} `;
    }
  }
  })
  // const result = calendar.map(day => {
  //   if (day.open === 0) {
  //     console.log(` ${day}: 'CLOSED'`)
  //     return ` ${day}: 'CLOSED'`;
  //   }else {
  //     console.log(` ${day}: Open from ${day.open}am util ${day.close} `)
  //     return ` ${day}: Open from ${day.open}am util ${day.close} `;
  //   }
  // })
  // let result ="";
  ...
  //     return  ` ${day}: Open from ${day.open}am util ${day.close} `;
  //   })
  console.log(Object.getOwnPropertyNames(calendar))
for (const [key, value] of Object.entries(calendar)) {
  // console.log(`${key}: ${value.open} ${value.close}`);
}
return result;
}
