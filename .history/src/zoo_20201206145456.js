...
function schedule(dayName) {
  ...
  for (const [key, value] of Object.entries(calendar)) {
    if (value.open === 0 && key === day) {
      // console.log(` ${key}: 'CLOSED'`)
      return result=result.concat(` ${key}: 'CLOSED'`);
    }else if(key === day) {
      // console.log(` ${key}: Open from ${value.open}am util ${value.close} `)
      return result=result.concat(` ${key}: Open from ${value.open}am util ${value.close} `);
    }
  }
  })
