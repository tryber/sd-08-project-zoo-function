...
function schedule(dayName) {
  ...
  const xblau = Object.entries(hours).reduce((day,hour) => {
    console.table(day);
    console.table(hour);
    console.log(`value: ${day.[].valueOf()}`);
    console.log(`open: ${day[1].open}`);
    if(day.key === dayName){
      if (day[1].open === 0) {
        result[day.valueOf()] = `CLOSED` ;
        return 0;
      }
        result[day.valueOf()] =  `Open from  ${day[1].open}am until  ${day[1].close}pm`;
        return 1;
    } else {
      if (day[1].open === 0) {
        result[day.valueOf()] = `CLOSED` ;
